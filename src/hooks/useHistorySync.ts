...
/**
 * History Cloud Sync — orchestration hook.
 *
 * Wires `useHistory.ts`'s local, localStorage-backed store to
 * `src/lib/supabase/historySync.ts`'s Supabase read/write layer, entirely
 * additively: nothing about `useHistory()`'s existing API changed, and any
 * component that only reads/writes history through it (HistoryPanel,
 * Calculator's evaluation-recording effect, the JSON import flow, …)
 ...
 *   or Supabase isn't configured — guest history stays exactly what it
 *   always was, localStorage-only, synced across tabs but never the cloud.
 * - **Restore after login** (once per signed-in session/user id): fetches
 *   this user's cloud history and merges it into the local store via
 *   `useHistory().mergeCloudItems()`, which preserves each item's id so a
 *   subsequent push upserts in place. This also naturally carries forward
 *   any history that was created locally *before* signing in (e.g. as a
 ...
 * - **Automatic sync while signed in**: any change to the local history
 *   (add/remove/edit-label/clear/import) is mirrored to the cloud on a
 *   short debounce, via `pushHistoryToCloud()`'s full-mirror upsert+delete.
 * - **Never blocks or throws.** A network failure, an unconfigured
 *   Supabase project, or an RLS/auth error just leaves `status` as
 *   `"error"` — the calculator and its local history keep working exactly
...
export type HistorySyncStatus =
  ...
  | "idle"
  /** Fetching + merging cloud history right after sign-in. */
  | "restoring"
  /** Pushing a local change up to the cloud. */
  | "syncing"
  /** Local and cloud history are known to match as of the last sync. */
  | "synced"
...
/* ---------------------------------------------------------------------
 * Shared, read-only status store — lets `useHistorySyncStatus()` (below,
 * used by the new Cloud Backup UI in Task 19) expose this hook's status
 * to any number of consumers without mounting a second, duplicate copy of
 * the restore/push effects. Same pattern `useSettingsSync.ts` already
 * established for its own shared status store.
 * ------------------------------------------------------------------- */
let sharedStatus: HistorySyncStatus = isSupabaseConfigured() ? "idle" : "disabled";
const statusListeners = new Set<() => void>();

function setSharedStatus(next: HistorySyncStatus): void {
  sharedStatus = next;
...
function getStatusSnapshot(): HistorySyncStatus {
  return sharedStatus;
}

function getStatusServerSnapshot(): HistorySyncStatus {
  return isSupabaseConfigured() ? "idle" : "disabled";
}

/**
 * Read-only subscription to the single `useHistorySync()` instance's status
 ...
 */
export function useHistorySyncStatus(): HistorySyncStatus {
...
export function useHistorySync(): UseHistorySyncReturn {
  const auth = useAuth();
  const { items, mergeCloudItems } = useHistory();
  const configured = isSupabaseConfigured();
  const [status, setStatus] = useState<HistorySyncStatus>(configured ? "idle" : "disabled");

  const applyStatus = useCallback((next: HistorySyncStatus) => {
    setStatus(next);
    setSharedStatus(next);
  }, []);

  // Tracks the user id this hook has already restored cloud history for, so
  // a re-render (e.g. a new history item being added) doesn't re-trigger a
  ...
  // Restore-on-login: fetch cloud history once per signed-in user id, merge
  // it into the local store (preserving ids), then push the merged result
  // back up so a local-only, pre-login item is uploaded too.
  useEffect(() => {
    ...
    async function restore(uid: string): Promise<void> {
      ...
      const result = await fetchCloudHistory(supabase, uid);
      if (cancelled) return;
      ...
      const pushResult = await pushHistoryToCloud(supabase, uid, merged);
      if (cancelled) return;
      applyStatus(pushResult.ok ? "synced" : "error");
      if (pushResult.ok) recordBackupSuccess("history");
    }
  ...
  // Ongoing sync: any local history change, while signed in, is mirrored to
  // the cloud on a short debounce (so rapid edits — e.g. typing a label —
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      applyStatus("syncing");
      const supabase = createClient();
      void pushHistoryToCloud(supabase, userId, items).then((result) => {
        applyStatus(result.ok ? "synced" : "error");
        if (result.ok) recordBackupSuccess("history");
      });
    }, PUSH_DEBOUNCE_MS);
  ...
  // badge showing.
  const effectiveStatus: HistorySyncStatus = !userId
    ? configured
      ? "idle"
      : "disabled"
    : status;

  // Keep the shared, read-only store (for `useHistorySyncStatus()`
